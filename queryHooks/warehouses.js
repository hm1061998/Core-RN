...
import { query, info, del, add, update } from '~/services/warehouses';
import {
  fetchDataQueries,
  updateQueryData,
  addQueryData,
...
export const useData = (init = {}, options = {}) => {
  const { params, key = 'warehouses' } = init;
  return useQuery(key, () => query(params), options);
};

export const useInfinitiData = (init = {}, options = {}) => {
  const { params, pageSize = 1, key = 'infiniti-warehouses' } = init;
