...
import { query, info, del, add, update } from '~/services/groupMedicines';
import {
  fetchDataQueries,
  updateQueryData,
  addQueryData,
...
export const useData = (init = {}, options = {}) => {
  const { params, key = 'groupMedicines' } = init;
  return useQuery(key, () => query(params), options);
};

export const useInfinitiData = (init = {}, options = {}) => {
  const { params, pageSize = 1, key = 'infiniti-groupMedicines' } = init;
...
export const useInfo = (init = {}, options = {}) => {
  ...
  const { id, key = 'groupMedicine' } = init;
  // console.log({ id });
