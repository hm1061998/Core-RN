...
import { query, info, del, add, update } from '~/services/producers';
import {
  fetchDataQueries,
  updateQueryData,
  addQueryData,
...
export const useData = (init = {}, options = {}) => {
  const { params, key = 'producers' } = init;
  return useQuery(key, () => query(params), options);
};

export const useInfinitiData = (init = {}, options = {}) => {
  const { params, pageSize = 1, key = 'infiniti-producers' } = init;
