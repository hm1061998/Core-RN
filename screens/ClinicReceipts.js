...
import { clinicReceipts } from '~/queryHooks';
...
import FilterClinicReceipt from '~/components/ClinicReceipts/FilterClinicReceipt';
...
const ClinicReceipts = ({ navigation, ctx, route }) => {
  ...
  const {
    ...
    isFetchingNextPage,
    isLoading,
  } = clinicReceipts.useInfinitiData({
    params: {
      filter: JSON.stringify({ ...filter }),
      sort: JSON.stringify(getValueSort(sortValue)),
    },
    pageSize: PAGE_SIZE,
    key: `clinicReceipts-sort-${sortValue}-${JSON.stringify(filter)}`,
  });
  ...
  const renderChangeStatus = activeTab => {
    ...
    switch (activeTab) {
      case true:
        return { status: 'Kích hoạt', color: '#f5222d' };
      case false:
        return { status: 'Hủy', color: '#fa8c16' };
      ...
      results.forEach(item => {
        let total =
          ...
          item.detail.reduce(
            (prev, chil) =>
              prev +
              Number(chil.price) * Number(chil.quantities) -
              Number(chil.discount) +
              Number(chil.vat),
            0,
          );
        ...
        const newItem = {
          ...item,
          total,
          debt: item.totalMoney - item.totalAmountOfMoneyPaid,
        };
  ...
  return (
    <View style={tw`flex-1 bg-THEME pt-status-bar`}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Phiếu thu dịch vụ</Text>
      ...
      <TouchableHighlight
        onPress={() => {
          navigation.navigate('CreateOrEditClinicReceipt');
        }}
        style={styles.addBtn}
      ...
      <FilterClinicReceipt ref={refFilter} />
    </View>
  );
};
...
export default withMenuContext(ClinicReceipts);
