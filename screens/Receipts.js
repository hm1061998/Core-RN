...
import FilterReceipts from '~/components/Receipts/FilterReceipts';
...
const Receipts = ({ navigation, ctx, route }) => {
  ...
  const {
  ...
  } = goodsReceipts.useInfinitiData(
    {
      ...
      key: `goodsReceipts-${sortValue}-${JSON.stringify(paramsFilter)}`,
    },
    // {
    //   enabled: false,
    // },
  ...
  const getTotalPriceItems = item => {
    let total =
      (item?.detail?.length > 0 &&
        item?.detail.map(
          e =>
            Number(e.price || 0) * Number(e.quantitiesReceived || 0) -
            Number(e.discount || 0) +
            Number(e.vat || 0),
        )) ||
      0;
  ...
  return (
    <View style={tw`flex-1 bg-THEME pt-status-bar`}>
          ...
          <FlashList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate('InfoReceipt', {
                    dataId: item.id,
                  })
                }>
      ...
      <TouchableHighlight
        onPress={() => {
          navigation.navigate('CreateOrEditReceipt');
        }}
